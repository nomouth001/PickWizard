...
from algorithm8 import prepare_sequences, train_lotto_model, predict_lotto, generate_5_sets_from_probs
...
def run_algorithm8(window_size, num_epochs, csv_path="lotto_data_.csv"):
    df = load_lotto_data(csv_path)
    total_rounds = len(df)
    now_str = datetime.now().strftime("%y%m%d%H%M%S")
    result_filename = f"lotto_result_algrm8_{now_str}.csv"

    model = None  # 초기에는 학습된 모델이 없음
    ...
    with open(result_filename, "w", newline="", encoding="utf-8") as f:
        ...
        for i in range(window_size, total_rounds):
            ...
            # [수정된 부분]: num_epochs를 사용자 입력값으로
            model = train_lotto_model(model, sequences, num_epochs=num_epochs)
...
if __name__ == "__main__":
    try:
        ...
        num_epochs = int(input("num_epochs를 입력하세요 (예: 1, 2, 5): ").strip())
        run_algorithm8(window_size, num_epochs)
