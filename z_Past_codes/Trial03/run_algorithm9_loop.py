...
from algorithm9 import prepare_sequences, train_lotto_model, predict_lotto, generate_5_sets_from_reversed_probs
...
def run_algorithm9(window_size, num_epochs, csv_path="lotto_data_.csv"):
    df = load_lotto_data(csv_path)
    total_rounds = len(df)
    now_str = datetime.now().strftime("%y%m%d%H%M%S")
    result_filename = f"lotto_result_algrm9_{now_str}.csv"
...
if __name__ == "__main__":
    try:
        ...
        run_algorithm9(window_size, num_epochs)
