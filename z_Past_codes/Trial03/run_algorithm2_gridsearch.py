...
def run_algorithm2_gridsearch(csv_path="lotto_data_.csv"):
    df = load_lotto_data(csv_path)
    total_rounds = len(df)

    now_str = datetime.now().strftime("%y%m%d%H%M%S")
    result_filename = f"lotto_gridsearch_algorithm2_{now_str}.csv"

    header = [
        "window_size", "1등", "2등", "3등", "4등", "5등", "2개맞음", "1개맞음", "0개맞음",
    ...
    with open(result_filename, "w", newline="", encoding="utf-8") as f:
        ...
        for window_size in range(1, total_rounds):
            ...
            for i in range(window_size, total_rounds):
                probs = calculate_probabilities(csv_path=csv_path, end_round=i, window_size=window_size)
            ...
            print(f"window_size {window_size} 완료")

    print(f"\n모든 결과 저장 완료: {result_filename}")

if __name__ == "__main__":
    run_algorithm2_gridsearch()
