...
from algorithm2 import calculate_probabilities, generate_5_sets_from_probabilities
...
def get_sorted_numbers_most_common(df, up_to_round):
    numbers = df[df['회차'] <= up_to_round][['번호1', '번호2', '번호3', '번호4', '번호5', '번호6']].values.flatten()
    counts = np.zeros(45)
    for num in numbers:
        counts[int(num) - 1] += 1
    sorted_numbers = np.argsort(-counts) + 1  # 높은 빈도순
...
def get_win_number_ranks(winning_numbers, counts_sorted):
    ...
    for num in winning_numbers:
        rank = counts_sorted_list.index(num) + 1  # 순위는 1부터 시작
        ranks.append(rank)
    return " ".join(map(str, ranks))

def compare_sets_to_winning_numbers(predict_sets, winning_numbers, bonus_number, algorithm_id, round_no, most_common_numbers, ranks_info):
    results = []
    for idx, pset in enumerate(predict_sets, start=1):
        ...
        result = {
            ...
            "일치번호": matched_str,
            "등수": rank,
            "빈도순정렬": most_common_numbers,
            "당첨번호순위": ranks_info
        }
        results.append(result)
    return results

def run_algorithm2_windowed(window_size, csv_path="lotto_data_.csv"):
    df = load_lotto_data(csv_path)
    total_rounds = len(df)
    start_round = window_size
    now_str = datetime.now().strftime("%y%m%d%H%M%S")
    result_filename = f"lotto_result_algrm2_{now_str}.csv"

    with open(result_filename, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["회차", "세트번호", "예측번호", "당첨번호", "일치개수", "일치번호", "등수", "빈도순정렬", "당첨번호순위"])

        for i in range(start_round, total_rounds):
            probs = calculate_probabilities(csv_path=csv_path, start_round=1, end_round=i)
            predict_sets = generate_5_sets_from_probabilities(probs)
            ...
            counts, sorted_numbers = get_sorted_numbers_most_common(df, up_to_round=i)
            most_common_numbers = " ".join(map(str, sorted_numbers))
            ranks_info = get_win_number_ranks(winning, sorted_numbers)

            results = compare_sets_to_winning_numbers(predict_sets, winning, bonus, algorithm_id=2, round_no=round_no, most_common_numbers=most_common_numbers, ranks_info=ranks_info)

            for r in results:
                writer.writerow([
                    r["회차"], r["세트번호"], r["예측번호"], r["당첨번호"],
                    r["일치개수"], r["일치번호"], r["등수"], r["빈도순정렬"], r["당첨번호순위"]
                ])
...
if __name__ == "__main__":
    try:
        ...
        run_algorithm2_windowed(window_size)
